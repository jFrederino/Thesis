...
class DAC_Table:
    ...
    def _linear_extrapolation(self) -> str:
        res = self.interpolate_value+1
        delta = 0.004499549999999999 / res
        new_fm,new_bm,new_ph,new_soa,new_wl  = [],[],[],[],[]
            ...
            for i in range(discontinuities_controller_indices.size): 
                ...
                if len(y) != 1:
                    ...
                    for i in range(0, len(x)-1):
                        next_x = x[i+1]
                        interp_wavelengths = [] #for each gap we interpolate
                        interp_wavelengths.append(x[i])
                        for k in range(1, res):
                            interp_wavelengths.append(x[i] + (delta * k)) #starts at k = 0 so it keeps the original points

                        interp_y = np.interp(interp_wavelengths, [x[i], next_x], [y[i], y[i+1]])
                        interp_values.extend(interp_y)
                    ...
                    for i in range(len(extrapolated_values)):
                        try: extrapolated_values[i] = interp_values[i]
                        except: pass
    ...
    def generate_DAC_table(self, interpolate_type: str = "true_linear", start_index:int = 0, end_index:int = 9999, plot_choice:bool = True, plot_both:bool = False) -> str:
        '''
        ...
        if interpolate_type == "line_fit":
            new_table_path = self._line_fit()

        if interpolate_type == "linear_extrapolation":
            new_table_path = self._linear_extrapolation()
