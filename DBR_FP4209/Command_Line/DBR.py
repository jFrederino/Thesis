...
class DBR_Spectrometer:
    ...
    '''
    def __init__(self, port_name:str = "COM4", mode: str = "manual", start_index: int = 0, end_index: int = 9999, interpolation_type: str = "true_linear", 
        interpolation_value:int = 3, plot_choice: bool = False, plot_both: bool = False,
        delay: float = 0.1, sending_packets: bool = True, sanatize: bool = True, log_voltage: bool = False, saving_LUT: bool = True, gui:bool = False, **port):
            self.mode = mode
            ...
            self._laser_serial = port



        
    def get_user_input(self, message:str, input_type:str):
        '''
        Parameters
        ----------
        message : str        
        ...
        if fastnumbers.check_int(user_input): user_input_type = "int"
            ...
            if input_type == "y/n": 
                if user_input.lower() not in ['y','n']:
                    ...
                    return self.get_user_input(message, input_type)
    ...
    def _get_interpolation_type(self, start_message):
            self.interpolation_type = self.get_user_input(message = start_message, input_type="str")
            if self.interpolation_type not in ["true_linear", "linear_extrapolation", "line_fit"]:
                self._get_interpolation_type(start_message="Please Input Interpolation Type ('true_linear'/'linear_extrapolation'/'line_fit'): ")
    ...
    def write_voltage(self):
        ...
        if self.interpolation_type == "true_linear":
            new_table_path = CWD+f'/Voltage_Data/True_Linear_({self.interpolation_value}, {self.start_index}, {self.end_index}).csv'

        if self.interpolation_type == "linear_extrapolation":
            new_table_path = CWD+f'/Voltage_Data/Linear_Extrapolation_({self.interpolation_value}, {self.start_index}, {self.end_index}).csv'

        if self.interpolation_type == "line_fit":
            new_table_path = CWD+f'/Voltage_Data/Line_Fit_({self.interpolation_value}, {self.start_index}, {self.end_index}).csv'
    ...
    def send_packets(self, DAC_list: list[list], manual: bool = True):
        ...
        if manual:
            if not self.get_user_input(message="Scan over full Interpolated DAC table Y/N: ", input_type="y/n"):
                num_packets = self.get_user_input(message="Number of Wavelengths to scan over (int): ", input_type="int")

        if manual:
            automatic = self.get_user_input(message="Scan automatically Y/N: ", input_type="y/n")
            if automatic: self.delay = self.get_user_input(message="Input Delay between packets: ", input_type="float")
        ...
        if not self.gui:
            for i in tqdm(range(len(packets))):
                ...
                if manual:
                    if not automatic and not self.get_user_input("Proceed to Send Y/N: ", input_type="y/n"):
    ...
    def _manual_setup(self):
        ...
        '''
        self.sending_packets = self.get_user_input(message="Send Packets to Laser Y/N: ", input_type="y/n")
        if self.sending_packets: 
            if not self._laser_connected: 
                self.connect_to_laser(target_port = self.get_user_input(message="Input Target Port: ", input_type="str"))

        self.log_voltage = self.get_user_input("Log Voltage Y/N: ", input_type="y/n")
        if self.log_voltage: 
        ...
        self.start_index = self.get_user_input(message="Start Index (int): ", input_type="int")
        self.end_index = self.get_user_input(message="End Index (int): ", input_type="int")
        self.interpolation_type = self._get_interpolation_type(start_message="Interpolation Type ('linear'/'curve_fit'): ")
        self.interpolation_value = self.get_user_input(message="Interpolation Value (int): ", input_type="int")
        self.plot_choice = self.get_user_input(message="Plot DAC values Y/N: ", input_type="y/n")
        self.plot_both = self.get_user_input(message="Plot Original values also Y/N: ", input_type="y/n")

        if self.get_user_input("Toggle Laser On Y/N: ", input_type="y/n"): self.enable()
