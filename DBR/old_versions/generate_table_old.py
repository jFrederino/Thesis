...
def get_DAC_arrays_from_table(csv_table_path:str, bounds:bool=False, start_index:int = 0, end_index:int = 9999, new_table: bool = "False"):
...
def generate_DAC_table(interpolate_type: str = "linear", interpolate_val:int = 0, start_index:int = 0, end_index:int = 9999, plot:bool = True, plot_both:bool = False):
    ...
    if interpolate_type == "curve_fit":
        ...
        #print(f"wl {len(wl)}")
        #print(f"ph {len(ph)}")
        #def interpolate(mode:str = "linear", controllers: list[list]):
        if interpolate_val > 0:
            for j in range(len(controllers)): 
                ...
                if interpolate_val: new_wl = w_values
                if plot:
